...
export const TerminalOutput = () => {
  ...
  const getLogColorClass = (log: string) => {
    ...
    if (log.includes("[Tool Error]") || log.includes("[Orchestrator Err]") || log.includes("[Python Err]") || log.includes("[Err]"))
    ...
    <div className='overflow-hidden bg-bg-secondary flex flex-col h-full w-full flex-1'>
      <div className='flex justify-between items-center px-4 py-3 bg-bg-tertiary border-b border-border-color/50 select-none text-[11px] font-mono text-text-muted shrink-0'>
        ...
        <div className='flex items-center gap-3.5'>
          ...
          <button
            className='hover:text-text-main hover:underline bg-transparent border-none text-text-muted text-[10px] font-mono tracking-wider cursor-pointer transition-all'
            onClick={clearLogs}
          >
            Clear Logs
          </button>
