        ...
        return (
          <div key={index} className="border border-border-color rounded-xl overflow-hidden bg-bg-tertiary/90 my-3 w-full max-w-full">
            <div className="flex justify-between items-center px-4 py-2 bg-bg-secondary border-b border-border-color/50 text-xs font-mono text-text-muted">
              ...
              <button
                className="hover:text-text-main hover:underline bg-transparent border-none text-[10px] font-mono tracking-wider cursor-pointer transition-all"
        ...
        <p key={index} className="whitespace-pre-wrap">
          {boldParts.map((bPart, bIndex) => {
            if (bPart.startsWith("**") && bPart.endsWith("**")) {
              return <strong key={bIndex} className="font-bold text-text-main">{bPart.slice(2, -2)}</strong>;
            }
            return bPart;
          })}
        </p>
      ...
      {messages.length === 0 ? (
        ...
        messages.map((msg) => (
          <div key={msg.id} className={`flex w-full ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[85%] sm:max-w-[75%] rounded-2xl p-4 flex flex-col gap-2 relative shadow-lg ${
              msg.role === "user"
              ...
              {msg.role === "assistant" && (
                <div className="flex items-center gap-2 mb-1 shrink-0">
                  ...
                  {msg.model && (
                    <span className={`px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wider border ${getModelBadgeClass(msg.model)}`}>
                      {msg.model}
                    </span>
                  )}
                </div>
        ...
        <div className="flex w-full justify-start animate-fade-in">
          <div className="max-w-[85%] sm:max-w-[75%] rounded-2xl p-4 flex flex-col gap-2 relative shadow-lg bg-bg-secondary border border-border-color rounded-tl-none text-text-main">
            ...
            <div className="flex items-center gap-1.5 py-2 px-1">
              ...
              <span className="w-2.5 h-2.5 rounded-full bg-accent-purple animate-bounce duration-500" style={{ animationDelay: "0.15s" }}></span>
              <span className="w-2.5 h-2.5 rounded-full bg-accent-blue animate-bounce duration-700" style={{ animationDelay: "0.3s" }}></span>
            </div>
          </div>
        </div>
      )}
